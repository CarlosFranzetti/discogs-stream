...
type ResolvedMedia =
  ...
  | { provider: 'bandcamp'; bandcampEmbedSrc: string; bandcampUrl?: string }
  | { provider: null };
...
export function useTrackMediaResolver(opts: { fetchRelease?: FetchRelease; discogsUsername?: string }) {
  ...
  const resolveMediaForTrack = useCallback(async (
    ...
    if (yt.length === 0) return { provider: null };
    ...
    if (best && best.videoId) return { provider: 'youtube', youtubeId: best.videoId };
