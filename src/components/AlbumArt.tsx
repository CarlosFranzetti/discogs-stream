...
export function AlbumArt({ track, isPlaying, showVideo, onClick }: AlbumArtProps) {
  return (
    <div
      className={`flex items-center justify-center h-full transition-opacity duration-500 cursor-pointer ${
        ...
        >
          ...
          <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
            <div className="bg-background/80 backdrop-blur-sm rounded-full px-3 py-2">
              <span className="text-xs text-muted-foreground">Click for video</span>
            </div>
          </div>
        </div>
      </div>
